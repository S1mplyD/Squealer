...
const createUserTable = async () => {
    ...
    if (me.status !== 404 && me.data.plan === "admin") {
        ...
        newMain.innerHTML =
            ...
            "<tr><th>Id</th><th>Name</th><th>Username</th><th>Mail</th><th>Plan</th><th>Status</th><th>Daily Characters</th><th>Weekly Characters</thWee><th>Monthly Characters</th></tr>" +
            "</table>";
        fillTable(users);
    } else {
...
const grantPermission = async (username) => {
    const response = await axios.patch("/api/users/grantPermissions", null, {
        params: { username },
    });
    return response.data;
};
const revokePermission = async (username) => {
    const response = await axios.patch("/api/users/revokePermissions", null, {
        params: { username },
    });
    return response.data;
};
