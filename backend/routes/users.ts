    ...
    .get(async (req: ExpressRequest, res: ExpressResponse) => {
        try {
            if (!req.user || (req.user as User).status !== "ban") {
                const users: SquealerError | User[] | null = await getAllUsers();
...
router.route("/user/:username/changeplan").post(async (req: ExpressRequest, res: ExpressResponse) => {
    try {
        if (
            req.user &&
            (req.user as User).status !== "ban" &&
...
router.route("/user/:username/addcharacters").post(async (req: ExpressRequest, res: ExpressResponse) => {
    try {
...
router.route("/unblock").post(async (req: ExpressRequest, res: ExpressResponse) => {
    try {
        if ((req.user as User).plan === "admin") {
...
router.route("/notification").get(async (req: ExpressRequest, res: ExpressResponse) => {
    if (
        req.user &&
...
router.route("/managedUsers/:username").get(async (req: ExpressRequest, res: ExpressResponse) => {
    try {
        if (
            (req.user as User).status !== "ban" &&
            (req.user as User).status !== "block"
...
router.route("/professionals").get(async (req: ExpressRequest, res: ExpressResponse) => {
    try {
        if (
            (req.user as User).status !== "ban" &&
            (req.user as User).status !== "block"
