...
router.route("/deleteOfficial/:name").delete(async (req, res) => {
    try {
        if (req.user && (req.user as User).plan === "admin") {
            const deleted: DeleteResult = await channelsModel.deleteOne({
                name: req.params.name,
            });
            if (deleted.deletedCount > 0) res.sendStatus(200);
            else res.sendStatus(500);
        } else res.sendStatus(401);
    } catch (e) {
        res.status(500).send(e);
